...
const CipherCreator: React.FC = () => {
  ...
  const encryptText = (text) => {
    ...
    // פיצול הטקסט למילים תוך שמירה על הרווחים המקוריים
    const words = text.split(/(\s+)/).filter(Boolean);

    if (language === 'he' && (selectedCipher === 'hieroglyphs' || selectedCipher === 'runes')) {
      // בעברית עבור הירוגליפים ורונות - הפוך את סדר המילים
      words.reverse();
    }

    // טיפול בכל מילה בנפרד
    const encryptedWords = words.map(word => {
      // אם זה רווח, נחזיר אותו כמו שהוא
      if (/^\s+$/.test(word)) return word;

      // בדיקה האם המילה מכילה רק מספרים
      const isNumber = /^\d+$/.test(word);

      let chars = word.split('');
      
      // הפיכת סדר האותיות לעברית ברונות והירוגליפים, אבל לא למספרים
      if (language === 'he' && !isNumber && (selectedCipher === 'hieroglyphs' || selectedCipher === 'runes')) {
        chars = chars.reverse();
      }
      
      // המרת כל תו לצופן המתאים
    ...
    // חיבור כל המילים חזרה לטקסט שלם
  ...
  return (
    <div className={`min-h-screen bg-gray-900 text-white p-8 font-sans ${language === 'he' ? 'rtl' : 'ltr'}`}>
      <div className="max-w-4xl mx-auto relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-8">
          {Object.entries(cipherStyles).map(([key, { icon: Icon, name, bgColor }]) => (
            <button
              key={key}
